...
public class NeuralLink {
	...
	public NeuralLink(IInputable p_input, IRecievable p_output){
		...
		m_weights = MatrixCalc.randomFloatMatrix(p_input.count(), p_output.count(), NeuralNetConfig.weight_mm);
	}

	//Create with specified weights. Weights must be right size
	public NeuralLink(IInputable p_input, IRecievable p_output, Matrix<float> p_weights){
		m_input = p_input;
		m_output = p_output;

		//Inputs = Rows, Outputs = columns
		
		if(m_input.count() != p_weights.RowCount || m_output.count() != p_weights.ColumnCount){
	...
	public void propagate(){		
		//Get inputs
		Matrix<float> propagation = m_input.getInputs();
